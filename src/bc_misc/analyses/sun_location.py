...
from tools import *
from xraysky import BC_Imager, sftime
...
if False:
    ...
    ph_all = sorted(top_dir.rglob("payload_checkout/CHECKOUT_26_06_2*/level1_tmp/ph_2*"))
...
for f in ph_all:
    ...
    try:
        ...
        t = sftime(ts_to_time(counts[0]['time']).item())
        wcs = WCS(imhdu.header)
        height,width = imhdu.data.shape
        # Where is Earth (relative to barycenter, close enough)
        antisunloc = astropy.coordinates.get_body(body='earth', time=t.to_astropy()).icrs
        # Why is this backward?
        # whysunloc = earth.at(t).observe(sun).to_skycoord().icrs
        # break
        # sunloc = (sun-earth).at(t).to_skycoord().icrs
        # # antisunloc = SkyCoord(ra=sunloc.ra + 180*u.deg, dec=-sunloc.dec, frame='icrs')
        sunij = [float(f) for f in wcs.world_to_pixel(antisunloc)]
        print(f"{obsname} antisun at {sunij[0] - width/2:.1f}, {sunij[1] - height/2:.1f}, ")
        continue
